<x-app-layout>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // =========================================================
            // FLAGS - Mencegah double submit
            // =========================================================
            let formChanged   = false;
            let formSubmitting = false;
            let isSubmitting  = false; // ← flag utama anti double submit
                    ...
                    if (checkbox.checked) {
                        ...
                        const shiftHtml = `
                            <div class="shift-wrapper space-y-4 mb-6 border border-gray-400 rounded-md p-3" data-shift="${shift}">
                                ...
                                <div id="max-quantity-display-${shift}" class="max-quantity-display-shift" style="display: none;"></div>
            ...
            function addAdditionalInputs(shiftWrapper, shift) {
                ...
                const additionalHtml = `
                    <div class="additional-inputs space-y-4 border border-gray-300 rounded-md p-3 mt-4">
                        ...
                        <div class="max-quantity-display-additional" style="display: none;"></div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Start Date</label>
                                <input type="date" name="start_dates[${shift}][]" value="${defaultStartDate}" required
            ...
            function addItemCodeAndQuantityListeners() {
                document.querySelectorAll('.item-code-selector').forEach(selector => {
                    ...
                    selector.addEventListener('change', function() {
                        ...
                        if (shiftWrapper.classList.contains('shift-wrapper')) {
                            ...
                            shift = shiftWrapper.closest('.shift-wrapper').getAttribute('data-shift');
                        }
                ...
                document.querySelectorAll('.quantity-input').forEach(input => {
                    if (input._listenerAttached) return; // ← cegah listener duplikat
                    ...
                    fetch("{{ route('calculate.item') }}", {
                        ...
                        .then(data => {
                            if (data.error) {
                                ...
                                itemCodeMaxQuantities[itemCode] = data.max_quantity;
            ...
            function addRemarkHandlers() {
                document.querySelectorAll('.remark-input').forEach(textarea => {
                    if (textarea._remarkListenerAttached) return; // ← cegah listener duplikat
                    textarea._remarkListenerAttached = true;
            ...
            // =========================================================
            // FORM SUBMIT - SINGLE LISTENER, ANTI DOUBLE SUBMIT
            // =========================================================
            form.addEventListener('submit', function(e) {
                ...
                if (submitButton) {
                    ...
                    // ✅ Fallback: reset otomatis setelah 15 detik jika ada network error
                    setTimeout(() => {
                        if (isSubmitting) {
                            isSubmitting          = false;
            ...
            // =========================================================
            // CHECKBOX & REMARK DYNAMIC LISTENER
            // =========================================================
            checkboxes.forEach(checkbox => {
