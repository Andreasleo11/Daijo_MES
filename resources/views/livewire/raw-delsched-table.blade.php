<div class="space-y-4">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-4 rounded-lg shadow-sm">
        <div class="flex-1">
            <input 
                type="text" 
                placeholder="Search by Item Code, SO Number..." 
            ...
            <table class="w-full border-collapse text-sm">
                <thead style="position: sticky; top: 0; background-color: #f3f4f6; z-index: 10;">
                    <tr class="border-b border-gray-300">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider cursor-pointer hover:bg-gray-200 whitespace-nowrap"
                            wire:click="sortBy('delivery_qty')">
                            Delivery Qty
                            @if($sortField === 'delivery_qty')
                                <span class="ml-1">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                            @endif
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider cursor-pointer hover:bg-gray-200 whitespace-nowrap"
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($rawData as $data)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="px-4 py-3 text-sm text-gray-700 font-medium whitespace-nowrap">{{ $data->item_code ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{{ $data->delivery_date ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700 font-medium whitespace-nowrap">{{ $data->delivery_qty ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700 font-medium whitespace-nowrap">{{ $data->so_number ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-4 rounded-lg shadow-sm">
        <div class="text-sm text-gray-600">
            Showing <span class="font-semibold">{{ $rawData->firstItem() ?? 0 }}</span> 
            to <span class="font-semibold">{{ $rawData->lastItem() ?? 0 }}</span> 
            of <span class="font-semibold">{{ $rawData->total() }}</span> entries
        </div>
