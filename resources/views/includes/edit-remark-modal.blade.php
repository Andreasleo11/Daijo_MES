<div id="edit-remark-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50">
    <div class="bg-white rounded-lg w-full max-w-lg p-6">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold">Edit Remark</h2>
            <button type="button" class="text-gray-600" onclick="closeModal('edit-remark-modal')">
        ...
        <form action="{{ route('remark.store', ['log_id' => $log->id]) }}" method="POST">
            ...
            <div class="mt-4 flex justify-end">
                <button type="button" class="px-4 py-2 text-white bg-gray-500 rounded-md mr-2"
                    onclick="closeModal('edit-remark-modal')">Cancel</button>
                <button type="submit" class="px-4 py-2 text-white bg-blue-500 rounded-md">Update
                    Remark</button>
            </div>
        </form>
    </div>
