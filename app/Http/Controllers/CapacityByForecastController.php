...
class CapacityByForecastController extends Controller
{
    ...
    public function step1(Request $request)
    {
		...
		$cnt_holiday_halfday = DB::table('holiday_schedules')->whereBetween('date',[$val_start_date , $val_end_date])->where('half_day',1)->count();
		
		$cnt_holiday_fullday = DB::table('holiday_schedules')->whereBetween('date',[$val_start_date , $val_end_date])->where('half_day',0)->count();
    ...
    public function step1_second(Request $request)
    {
		...
		foreach($tab_forecast_sorted as $fore_forecast_sorted){						
           ...
           //MASI ADA BUG JIKA DATRA tidak correct cycletime pasti akan null
			...
			$tab_line_list_one = DB::table('inv_line_list')->where('line_code',$val_line_code_first)->first();
    ...
    public function step2logic(Request $request)
    {
		...
		$tab_line_list = DB::table('inv_line_list')->get();
    ...
    {
        $tab_line_list_category = DB::table('inv_line_list')->select('category')->distinct()->get();
		
		foreach($tab_line_list_category as $line_category){
			
			$val_category = $line_category->category;
			
			$con_line_category = DB::table('inv_line_list')->where('category',$val_category)->count();
