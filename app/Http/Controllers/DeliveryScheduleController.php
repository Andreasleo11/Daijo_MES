...
use App\Models\Delivery\delsched_finalwip;
...
class DeliveryScheduleController extends Controller
{
	 ...
	 public function step1()
    {
        ...
        $log = DB::table('delsched_process_logs')->where('process_key', 'delsched_main')->first();
        if ($log && $log->status === 'running') {
            return response()->json(['message' => 'Proses masih berjalan, harap tunggu...', 'status' => 'running']);
        }

        // Set status awal
        DB::table('delsched_process_logs')->updateOrInsert(
            ['process_key' => 'delsched_main'],
            ['current_step' => 'step1', 'status' => 'running', 'message' => 'Proses dimulai...', 'updated_at' => now()]
        );

        // Dispatch job ke queue
        ProcessDelschedStep1::dispatch();

        return response()->json(['message' => 'Proses dimulai di background!', 'status' => 'running']);
    }

    // =============================================
    // TRIGGER: Jalankan proses WIP di background
    // =============================================
    public function step1wip()
    {
        $log = DB::table('delsched_process_logs')->where('process_key', 'delsched_wip')->first();
        if ($log && $log->status === 'running') {
            return response()->json(['message' => 'Proses WIP masih berjalan...', 'status' => 'running']);
        }

        DB::table('delsched_process_logs')->updateOrInsert(
            ['process_key' => 'delsched_wip'],
            ['current_step' => 'step1', 'status' => 'running', 'message' => 'Proses WIP dimulai...', 'updated_at' => now()]
        );

        ProcessDelschedWipStep1::dispatch();

        return response()->json(['message' => 'Proses WIP dimulai di background!', 'status' => 'running']);
    }
    ...
    // Step 2, 3, 4 (route lama tetap ada untuk kompatibilitas, tapi tidak dipakai langsung)
    public function step2() { return redirect()->route('indexds'); }
    public function step3() { return redirect()->route('indexds'); }
    public function step4() { return redirect()->route('indexds'); }
    public function step2wip() { return redirect()->route('indexfinalwip'); }
